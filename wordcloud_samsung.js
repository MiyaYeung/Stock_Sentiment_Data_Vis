...
//initialise canvas
var graph_wordcloud_right = d3.select("#svg-wordcloud-right")
.attr("width", width + margin.left + margin.right)
.attr("height", height + margin.top + margin.bottom)
.style("position", "relative")
.style("top", adj);

var plot_wordcloud_right = graph_wordcloud_right.append("g")
.attr("class", "plot-wordcloud")   
.attr("id", "plot-wordcloud-right")
.attr("transform", "translate(" + margin.left + "," + margin.top + ")");

//Append chart title
graph_wordcloud_right.append("text")
.attr("id","title-graph-wordcloud-right")
.attr("transform", "translate(100,0)")
.attr("x", 100)
.attr("y", 30)
.attr("font-size", "14px")
.attr("stroke", "black")
.text("Samsung Wordcloud"); 

//--------------- read data, draw wordcloud
// read data for wordcloud
d3.dsv(",", "wordcloud/wordcloud_samsung.csv", function(d) {
    ...
    // console.log("top 50 samsung sorted Text: ", top50sortedText)
    ...
    function draw(words) {
        var plot_wordcloud_right_enter = plot_wordcloud_right.append("g")
        .attr("transform", "translate(" + layout.size()[0] / 2 + "," + layout.size()[1] / 2 + ")")
        .selectAll("text")
        .data(words)
        .enter()
        
        var plot_wordcloud_right_enter_text = plot_wordcloud_right_enter.append("text")
    ...
    function drawTooltip(d) {
        ...
        // console.log("samsung data when mouseover", d)
